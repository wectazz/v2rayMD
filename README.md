# v2rayMD (fork of v2rayNG with Material Design 3 Expressive)
...
- geoip.dat and geosite.dat files are in `Android/data/com.v2ray.md/files/assets` (path may differ on some Android device)
...
- geoip.dat 和 geosite.dat 文件位于 `Android/data/com.v2ray.md/files/assets`（部分设备路径可能不同）
...
- Android project under the V2rayMD folder can be compiled directly in Android Studio, or using the Gradle wrapper. But the v2ray core inside the aar is (probably) outdated.
...
- v2rayMD can run on Android Emulators. For WSA, VPN permission needs to be granted via `appops set [package name] ACTIVATE_VPN allow`.

### 提示

- V2rayMD 文件夹下的 Android 项目可直接在 Android Studio 中编译，或使用 Gradle wrapper 编译。但 aar 内置的 v2ray core（可能）已过时。
...
- v2rayMD 可在 Android 模拟器上运行。对于 WSA，需要通过 `appops set [package name] ACTIVATE_VPN allow` 授予 VPN 权限。

---

## GPG Verification / GPG 签名校验

Release files are signed with GPG to verify authenticity and integrity, helping prevent mirror, ISP, or CDN hijacking. The signing key fingerprint is published in the release notes.

发布文件已使用 GPG 签名，可用于校验文件真实性与完整性，预防镜像站、运营商或 CDN 劫持。签名公钥指纹见发布说明。
